...
def add_note(request):
    ...
    if res == 1:
        ...
        Note.objects.create(title=title, text=text, start_time=s_time, end_time=e_time, grade=grade, status=now_status)
        # addLog(note.id, 'A')
...
def change_status(n_id, status):
    Note.objects.filter(uuid=n_id).update(status=status)
    if Note.objects.filter(uuid=n_id, status=status).exists():
        return 1
    return -1
...
def addLog(n_id, operate):
    ...
    Log.objects.create(uuid=n_id, operation=operate, record_time=current_time)
